(() => {
  ...
  // ../workspace/src/lib/offline/lexicon.ts
  ...
  // ../workspace/src/lib/offline/misspellings.ts
  var MISSPELLINGS = {
    "a": "aa",
    "aacdemic": "academic",
    "aacdemy": "academy",
  ...
  // ../workspace/src/lib/offline/instruction.ts
...
`.trim();
  ...
  // ../workspace/src/lib/offline/brain.ts
...
${b}`;
  ...
  // ../workspace/src/lib/offline/pack-info.ts
  var PACK_INFO = {
    lexiconWords: 19938,
    misspellings: 25206,
    version: "1.2.0"
  ...
  // ../workspace/src/lib/drill-bank.ts
  var DRILL_BANK = [
  ...
  // ../workspace/src/lib/syllables.ts
  ...
  // lexi-apk-app.js
  function boot() {
    var Lexi2 = window.Lexi;
    if (!Lexi2) {
      document.getElementById("app").innerHTML = "<div class='card'><p>The on-device pack did not load.</p><p>Reinstall LexiForge.</p></div>";
      return;
    }
    var KEY = "lexiforge-v1";
    var defaults = {
      temperature: 2,
      font: "atkinson",
      fontSize: 20,
    ...
    var chunkOn = false;
    var copied = false;
    var drill = { cards: [], i: 0, right: 0, done: false };
    function applyChrome() {
      var root2 = document.documentElement;
      root2.style.setProperty("--size", state.fontSize + "px");
      root2.style.setProperty("--track", state.letterSpacing + "em");
      root2.style.setProperty("--lead", String(state.lineHeight));
      root2.style.setProperty(
        "--font",
        state.font === "opendyslexic" ? "OpenDyslexic, Atkinson, sans-serif" : "Atkinson, sans-serif"
      );
    ...
    function tokensHtml(tokens) {
      return tokens.map(function(t) {
        ...
        return '<button type="button" class="mark" data-orig="' + escapeHtml(t.original.trim()) + '" data-hint="' + escapeHtml(t.hint || "") + '">' + escapeHtml(t.display) + "</button>";
      }).join("");
    }
    function today() {
    ...
    function touchStreak() {
      ...
      var yk = y.toISOString().slice(0, 10);
    ...
    function render() {
      ...
      if (route === "home") {
        ...
        var sug = "";
        if (state.suggestedTemp != null) {
          sug = '<div class="card"><p>Try ' + state.suggestedTemp + ' tomorrow? You choose.</p><div class="row"><button type="button" id="useSug">Use ' + state.suggestedTemp + '</button><button class="secondary" type="button" id="skipSug">Keep ' + state.temperature + "</button></div></div>";
        }
        el.innerHTML = '<p class="muted">Read. Write. Practice. Works on this device.</p><div class="card"><div class="temp-labels"><span>Clean 0</span><span>Easy</span><span>Train</span><span>Hard</span><span>Drill 10</span></div><input id="temp" type="range" min="0" max="10" step="0.5" value="' + state.temperature + '" /><p><strong>' + tempLabel(state.temperature) + "</strong> \xB7 " + state.temperature + "</p></div>" + sug + '<div class="stack"><button type="button" data-go="read">Help me read this<br><span class="muted">Paste a sign or a message</span></button><button class="secondary" type="button" data-go="chat">Talk / write with me<br><span class="muted">Short chat with a few marked words</span></button><button class="secondary" type="button" data-go="drill">2-minute drill<br><span class="muted">Quick cards. Then stop.</span></button></div><p class="muted">' + (state.streak ? state.streak + " day streak. " : "Start today. ") + (state.wordsPracticed ? state.wordsPracticed + " words practiced." : "") + '</p><p class="muted">Font</p><div class="row"><button class="' + (state.font === "atkinson" ? "" : "secondary") + '" type="button" data-font="atkinson">Atkinson</button><button class="' + (state.font === "opendyslexic" ? "" : "secondary") + '" type="button" data-font="opendyslexic">OpenDyslexic</button></div><label class="small">Size <input id="fs" type="range" min="18" max="28" step="1" value="' + state.fontSize + '" /></label><label class="small">Letter space <input id="ls" type="range" min="0.02" max="0.16" step="0.01" value="' + state.letterSpacing + '" /></label><label class="small">Line height <input id="lh" type="range" min="1.5" max="2.2" step="0.05" value="' + state.lineHeight + '" /></label><p class="muted">This is a practice tool and a reading helper. It does not diagnose or treat dyslexia. You control how hard it is. Use Clean mode whenever you need the real words.</p>';
      } else if (route === "read") {
        title.textContent = "Help me read this";
        speakText = cleanText ? "Help me read this. " + cleanText : "Help me read this. Paste a sign, a message, or a paragraph. Then tap Clean it.";
        var out = "";
        if (cleanText) {
          var shown = chunkOn ? Lexi2.chunkText(cleanText) : cleanText;
          out = '<div class="card"><p>' + escapeHtml(shown) + '</p><p class="muted">What this means</p><p>' + escapeHtml(meaningText || "") + '</p><button class="secondary" type="button" id="doCopy">' + (copied ? "Copied" : "Copy clean") + "</button></div>";
        }
        el.innerHTML = '<label>Paste the text<textarea id="paste" placeholder="A sign, a message, or a paragraph."></textarea></label><div class="stack"><button type="button" id="doClean">Clean it</button><button class="secondary" type="button" id="doSay">Say it</button><button class="secondary" type="button" id="doChunk">Chunk it</button></div><div id="out">' + out + "</div>";
        ...
        if (!chatMsgs.length) {
          chatMsgs.push({
            role: "assistant",
            text: Lexi2.OPENING,
            tokens: Lexi2.applyNoise(Lexi2.OPENING, state.temperature)
          });
        }
        var last = null;
        ...
        var log = chatMsgs.map(function(m) {
          if (m.role === "user") {
            return '<article class="card"><p>' + escapeHtml(m.text) + "</p></article>";
          }
          return '<article class="card">' + (m.userClean ? "<p>I think you mean: " + escapeHtml(m.userClean) + "</p>" : "") + "<p>" + tokensHtml(m.tokens || Lexi2.applyNoise(m.text, 0)) + "</p></article>";
        }).join("");
        el.innerHTML = '<p class="muted">' + (state.temperature >= 9 ? "Drill mode is on. Chat stays readable." : "Marked words are practice. Tap one to see the clean spelling. Lexi runs on this device.") + '</p><div id="log">' + log + '</div><div id="reveal"></div><form id="sendForm"><textarea id="box" rows="3" placeholder="Type here. Spelling can be messy."></textarea><div class="row" style="margin-top:0.5rem"><button type="submit">Send</button><button class="secondary" type="button" id="tooHard">Too hard</button></div></form>';
      } else if (route === "drill") {
        title.textContent = "2-minute drill";
        if (!drill.cards.length) drill.cards = Lexi2.pickDrill(10);
        var card = drill.cards[drill.i];
        if (drill.done || !card) {
          var acc = drill.cards.length ? Math.round(drill.right / drill.cards.length * 100) : 0;
          speakText = "Drill done. " + drill.right + " of " + drill.cards.length + " correct.";
          el.innerHTML = '<div class="card"><p>Done.</p><p>' + drill.right + " of " + drill.cards.length + " correct (" + acc + '%).</p><div class="stack"><button type="button" data-go="home">Home</button><button class="secondary" type="button" id="again">Try again</button></div></div>';
        } else {
          speakText = "Drill. " + card.prompt + " " + card.noisy;
          var body = '<p class="muted">' + (drill.i + 1) + " of " + drill.cards.length + '</p><div class="card"><p>' + escapeHtml(card.prompt) + "</p><p><strong>" + escapeHtml(card.noisy) + '</strong></p><p class="muted">' + escapeHtml(card.hint) + "</p></div>";
          if (card.options && card.options.length) {
            body += '<div class="stack">' + card.options.map(function(o) {
              return '<button class="opt" type="button" data-ans="' + escapeHtml(o) + '">' + escapeHtml(o) + "</button>";
            }).join("") + "</div>";
          } else {
            body += '<form id="typeForm"><input id="typed" style="width:100%;min-height:3rem;font:inherit;padding:0.6rem 0.8rem;border-radius:0.7rem;border:2px solid #cbbfa8" placeholder="Type the clean word" /><button type="submit" style="margin-top:0.5rem">Check</button></form>';
          }
          el.innerHTML = body;
        }
      }
    ...
    document.body.addEventListener("click", function(e) {
      var t = e.target.closest ? e.target.closest(
        "[data-go],[data-font],#back,#cleanToggle,#speakScreen,#doClean,#doSay,#doChunk,#doCopy,#tooHard,#useSug,#skipSug,#again,.mark,.opt"
      ) : null;
      if (!t) return;
      if (t.id === "back" || t.getAttribute("data-go") === "home") {
        route = t.getAttribute("data-go") || "home";
        render();
        return;
      }
      if (t.getAttribute("data-go")) {
        route = t.getAttribute("data-go");
        if (route === "drill") {
          drill.cards = Lexi2.pickDrill(10);
          drill.i = 0;
          drill.right = 0;
          drill.done = false;
        }
      ...
      if (t.getAttribute("data-font")) {
        state.font = t.getAttribute("data-font");
        save(state);
        render();
        return;
      }
      if (t.id === "useSug") {
        state.temperature = state.suggestedTemp;
        state.suggestedTemp = null;
        save(state);
        render();
        return;
      }
      ...
      if (t.id === "doClean") {
        ...
        var r = Lexi2.interpretOnDevice(text);
        cleanText = r.clean;
        meaningText = r.meaning;
        chunkOn = false;
        copied = false;
        touchStreak();
        state.wordsPracticed += 1;
        save(state);
        render();
        return;
      }
      if (t.id === "doSay") {
        var paste2 = document.getElementById("paste");
        speak(cleanText || (paste2 ? paste2.value : "") || "");
        return;
      }
      if (t.id === "doChunk") {
        chunkOn = !chunkOn;
      ...
      if (t.id === "tooHard") {
        state.suggestedTemp = Math.max(0, state.temperature - 1);
        save(state);
        route = "home";
        render();
        return;
      ...
      if (t.classList.contains("mark")) {
        ...
        if (box) {
          box.innerHTML = '<div class="card"><p>Yes \u2014 ' + escapeHtml(t.getAttribute("data-orig") || "") + '. Nice catch.</p><p class="muted">' + escapeHtml(t.getAttribute("data-hint") || "") + "</p></div>";
          state.wordsPracticed += 1;
          save(state);
          touchStreak();
        }
      ...
      if (t.classList.contains("opt")) {
        ...
        var ok = String(t.getAttribute("data-ans") || "").toLowerCase() === c.answer.toLowerCase();
        if (ok) drill.right += 1;
        ...
        setTimeout(function() {
          drill.i += 1;
          if (drill.i >= drill.cards.length) {
            drill.done = true;
            if (drill.right / drill.cards.length < 0.6) {
              state.suggestedTemp = Math.max(0, state.temperature - 1);
              save(state);
            }
          }
    ...
    document.body.addEventListener("input", function(e) {
      if (e.target.id === "temp") {
        ...
        if (p) {
          p.innerHTML = "<strong>" + tempLabel(state.temperature) + "</strong> \xB7 " + state.temperature;
        }
      }
      if (e.target.id === "fs") {
    ...
    document.body.addEventListener("submit", function(e) {
      if (e.target.id === "sendForm") {
        ...
        var local = Lexi2.coachOnDevice(text, chatTurn++);
        chatMsgs.push({
          role: "assistant",
          text: local.reply,
          userClean: local.userClean,
          tokens: Lexi2.applyNoise(local.reply, state.temperature)
        });
        render();
      }
      if (e.target.id === "typeForm") {
        ...
        var typed = (typedEl && typedEl.value || "").trim().toLowerCase();
  ...
  window.onerror = function(msg) {
    var el = document.getElementById("app");
    if (el) {
      el.innerHTML = "<div class='card'><p>Start failed.</p><p>" + String(msg) + "</p></div>";
    }
  };
